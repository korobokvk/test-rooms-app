...
// Paths
const { Login, Logout } = FullPaths.Auth;
...
describe('AuthRouter', () => {
  ...
  describe(`"POST:${Login}"`, () => {
    ...
    const callApi = (reqBody: TReqBody) => agent.post(Login).type('form').send(reqBody);

    // Success
    it(
      `should return a response with a status of "${OK}" and a cookie with ` + 'a jwt if the login was successful.',
      (done) => {
        // Setup data
  ...
  describe(`"GET:${Logout}"`, () => {
    ...
    it(`should return a response with a status of ${OK}`, (done) => {
      agent.get(Logout).end((_: Error, res: Response) => {
