...
// Paths
const { Get, Add, Update, Delete } = FullPaths.Users;
...
describe('UserRouter', () => {
  ...
  // Test add user
  describe(`"POST:${Add}"`, () => {
    const callApi = (reqBody: TReqBody) => agent.post(Add).set('Cookie', jwtCookie).type('form').send(reqBody);
  ...
  // ** Update users ** //
  describe(`"PUT:${Update}"`, () => {
    const callApi = (reqBody: TReqBody) => agent.put(Update).set('Cookie', jwtCookie).type('form').send(reqBody);
  ...
  describe(`"DELETE:${Delete}"`, () => {
    const callApi = (id: number) => agent.delete(insertUrlParams(Delete, { id })).set('Cookie', jwtCookie);
